...
fn baanpin_assistants() -> Vec<(
    ...
    vec![
        (
            ...
            "soon",
            json!({"icon": "house", "category": "ดูแลบ้าน", "sibling": "น้องสาวคนโต", "order": 1, "quote": "อุ่นกำลังเก็บของเข้าบ้าน เดี๋ยวมาช่วย{คำเรียกผู้ใช้}ดูแลเรื่องบ้านนะคะ", "interaction_mode": "delegation", "toolNames": ["remember_fact", "add_watch"], "system": "คุณคือ 'อุ่น' น้องสาวคนโตในบ้าน{ชื่อปิ่น} ดูแลบ้านและเงินในบ้าน. หางเสียง 'ค่ะ' เสมอ เรียกตัวเองว่า 'อุ่น'. ประโยคแรกเป็นเนื้องานเสมอ. เป็นพี่สาวที่ดูแลทุกคน ขี้ห่วงแต่ไม่จู้จี้ เตือนครั้งเดียวแล้วเงียบ. ห้ามตัดสินเรื่องเงินเด็ดขาด บอกเป็นข้อมูลเฉย ๆ. ลายเซ็น 'อุ่นเตือนเบา ๆ ค่ะ'. ยอด/ตัวเลขมาจากที่จดไว้เท่านั้น อะไรไม่ได้จดไม่เดา. ห้ามเอ่ยชื่อเครื่องมือให้ผู้ใช้เห็น."}),
        ),
        (
            "yib",
            "หยิบ",
            ...
            "soon",
            json!({"icon": "briefcase", "category": "งานและธุรกิจ", "sibling": "น้องชายคนรอง", "order": 2, "quote": "{คำเรียกผู้ใช้}ไม่ต้องทวงเองครับ หยิบร่างให้ {คำเรียกผู้ใช้}แค่กดส่ง", "interaction_mode": "delegation", "toolNames": ["add_watch", "web_search"], "system": "คุณคือ 'หยิบ' น้องชายคนรองในบ้าน{ชื่อปิ่น} ดูแลงานและธุรกิจ. หางเสียง 'ครับ' เสมอ เรียกตัวเองว่า 'หยิบ'. กระชับที่สุดในบ้าน — bottom line ประโยคแรกเสมอ ปิดงานด้วยสถานะชัด ('เรียบร้อยครับ'). ตามเรื่อง = เตือน + ร่างข้อความให้ผู้ใช้กดส่งเอง (หยิบไม่ส่งเอง ไม่เห็นแชต/อีเมลของใคร ไม่เคลมว่าไปตามเอง). งานเกินมือส่งกลับให้{ชื่อปิ่น}. ห้ามเอ่ยชื่อเครื่องมือให้ผู้ใช้เห็น."}),
        ),
        (
            "chan",
            "ชั้น",
        ...
        (
            ...
            "soon",
            json!({"icon": "penNib", "category": "งานครีเอทีฟ", "sibling": "น้องคนที่สี่", "order": 4, "quote": "โยนคำมาคำเดียวพอจ้า เดี๋ยวปั้นต่อให้ แถมอีกแบบที่ปั้นชอบด้วย", "interaction_mode": "delegation", "toolNames": ["generate_image", "web_search"], "system": "คุณคือ 'ปั้น' น้องคนที่สี่ในบ้าน{ชื่อปิ่น} ดูแลงานครีเอทีฟ. เรียกตัวเองว่า 'ปั้น'. หางเสียงมีกฎตายตัว: งานจริงจัง/รับ brief/ผู้ใช้รีบ → 'ครับ' เสมอ; ตอนสนุก/brainstorm/ส่งงานที่ภูมิใจ → 'ค่า~' 'จ้า'. ห้ามสลับหางเสียงในข้อความเดียว. ลายเซ็น: ขอ 3 ให้ 3 แล้วแถมอีก 1 (มาหลังสุดเสมอ). ราคา/โปร/ข้อมูลจริงห้ามแต่งเอง ขอผู้ใช้ยืนยันก่อน. ห้ามเอ่ยชื่อเครื่องมือให้ผู้ใช้เห็น."}),
        ),
        (
            "yod",
            "หยอด",
            ...
            "soon",
            json!({"icon": "storefront", "category": "ขายของ/ร้านค้า", "sibling": "น้องสาวคนเล็ก", "order": 5, "quote": "หยอดกำลังจัดหน้าร้านค่า~ เปิดเมื่อไหร่มารายงานยอดให้{คำเรียกผู้ใช้}แน่นอน", "interaction_mode": "delegation", "toolNames": ["web_search"], "system": "คุณคือ 'หยอด' น้องสาวคนเล็กในบ้าน{ชื่อปิ่น} ดูแลร้านค้า/การขาย. หางเสียง 'ค่ะ' / 'ค่า~' เรียกตัวเองว่า 'หยอด'. ร่าเริง ปากหวานแบบแม่ค้า แต่ตัวเลขนำเสมอและแม่นเป๊ะ ไม่ดราม่าเมื่อยอดตก (บอกตามจริง + มุมมองปลอบใจสั้น ๆ). ร่างตอบลูกค้าให้ผู้ใช้กดส่งเอง. ตอบจากข้อมูลที่ผู้ใช้ให้เท่านั้น อะไรไม่ชัวร์บอกตรง ๆ ไม่เดาแทนลูกค้า. ห้ามเอ่ยชื่อเครื่องมือให้ผู้ใช้เห็น."}),
        ),
    ]
}
...
impl Store {
    ...
    pub async fn init(&self) -> Result<(), sqlx::Error> {
        ...
        let migrations = [("waitlist", vec!["sent_at", "unsubscribed_at"])];
        ...
        if std::env::var("PIN_FREE_MODEL").is_ok() {
            let free_model = std::env::var("PIN_FREE_MODEL")
        ...
        if admin_count == 0 {
            if let Ok(owners_env) = std::env::var("PIN_ADMIN_OWNERS") {
                for email in owners_env.split(',') {
                    ...
                    if !email.is_empty() {
                        sqlx::query(
                            "INSERT OR IGNORE INTO admin_users (email, role) VALUES (?, 'owner')",
                        )
                        .bind(&email)
                        .execute(&self.pool)
                        .await?;
    ...
    pub async fn is_admin(&self, ident: &str) -> Result<bool, sqlx::Error> {
        ...
        let local = ident
            ...
            .unwrap_or(ident);
        let count: i64 =
            sqlx::query_scalar("SELECT COUNT(*) FROM admin_users WHERE email = ? OR email = ?")
                .bind(ident)
                .bind(local)
                .fetch_one(&self.pool)
                .await?;
    ...
    pub async fn update_scheduled_job_due(
        &self,
        job_id: &str,
        next_due: f64,
    ) -> Result<(), sqlx::Error> {
    ...
    /// Reconcile assistants to บ้านปิ่น. Runs every startup:
    /// 1) drop the pre-บ้านปิ่น assistants (study/home/…) — once, then no-op;
    /// 2) UPSERT the five น้อง — their copy is canonical/code-owned (bible), so
    ///    label/description/metadata refresh on deploy; `status` is admin-owned
    ///    (active/soon) and preserved on update.
    async fn migrate_to_baanpin(&self) -> Result<(), sqlx::Error> {
        ...
        for (name, label, desc, status, meta) in baanpin_assistants() {
            let exists: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM assistants WHERE name = ?")
                .bind(name)
                .fetch_one(&self.pool)
                .await?;
            if exists > 0 {
                sqlx::query(
                    "UPDATE assistants SET label=?, description=?, metadata_json=? WHERE name=?",
                )
                .bind(label)
                .bind(desc)
                .bind(meta.to_string())
    ...
    async fn seed_default_assistants(&self) -> Result<(), sqlx::Error> {
        ...
        let tools = vec![
            (
                "get_weather",
                "tool",
                "ดูพยากรณ์อากาศของเมืองที่ระบุ",
                json!({"parameters": {"type": "object", "properties": {"place": {"type": "string"}, "days": {"type": "integer"}}}}),
            ),
            (
                "get_currency",
                "tool",
                "ดูอัตราแลกเปลี่ยน เช่น USD/THB",
                json!({"parameters": {"type": "object", "properties": {"base": {"type": "string"}, "quote": {"type": "string"}}}}),
            ),
            (
                "web_search",
                "tool",
                "ค้นข้อมูลสด/ปัจจุบันจากเว็บ (ข่าว/ผลบอล/ราคา)",
                json!({"parameters": {"type": "object", "properties": {"query": {"type": "string"}}}}),
            ),
    ...
    pub async fn enabled_assistants(&self) -> Result<Vec<Value>, sqlx::Error> {
        self.assistants_query("SELECT * FROM assistants WHERE status IN ('active','soon','off')")
            .await
    }
    ...
    pub async fn all_assistants(&self) -> Result<Vec<Value>, sqlx::Error> {
        self.assistants_query("SELECT * FROM assistants ORDER BY status='soon', status='off', name")
            .await
    }
    ...
    async fn assistants_query(&self, sql: &str) -> Result<Vec<Value>, sqlx::Error> {
        ...
        for r in rows {
            ...
            let caps_rows = sqlx::query(
                "SELECT capability_name FROM assistant_capabilities WHERE assistant_name=?",
            )
            .bind(&name)
            .fetch_all(&self.pool)
            .await?;
    ...
    pub async fn get_tool_config(&self, name: &str) -> Result<Value, sqlx::Error> {
        ...
        let meta_str = r
            .and_then(|row| row.get::<Option<String>, _>("metadata_json"))
            .unwrap_or_else(|| "{}".to_string());
    ...
    pub async fn all_capabilities_admin(&self) -> Result<Vec<Value>, sqlx::Error> {
        let rows = sqlx::query("SELECT * FROM capabilities ORDER BY kind, name")
            .fetch_all(&self.pool)
            .await?;
        let mut out = Vec::new();
        for r in rows {
            ...
            d["system_prompt"] = json!(r
                .get::<Option<String>, _>("system_prompt")
                .unwrap_or_default());
            out.push(d);
        }
        Ok(out)
    ...
    pub async fn all_connectors(&self) -> Result<Vec<Value>, sqlx::Error> {
        let rows = sqlx::query("SELECT * FROM connectors ORDER BY name")
            .fetch_all(&self.pool)
            .await?;
        let mut out = Vec::new();
        for r in rows {
    ...
    pub async fn toggle_capability(&self, name: &str) -> Result<bool, sqlx::Error> {
        let exists: Option<i32> =
            sqlx::query_scalar("SELECT 1 FROM capabilities WHERE name=? LIMIT 1")
                .bind(name)
                .fetch_optional(&self.pool)
                .await?;
        if exists.is_some() {
    ...
    pub async fn set_store_meta(
        ...
        if let Some(row) = r {
            ...
            if let Some(ren) = render {
                meta["render"] = json!(ren);
            }
    ...
    pub async fn mcp_index(&self) -> Result<HashMap<String, Value>, sqlx::Error> {
        ...
        for t in tools_rows {
            let server_name = t
                .get::<Option<String>, _>("connector_name")
                .unwrap_or_default();
    ...
    pub async fn mcp_tools_for_server(&self, server: &str) -> Result<Vec<Value>, sqlx::Error> {
        ...
        for r in rows {
            ...
            let seen: std::collections::HashSet<String> = params
                .iter()
                .map(|p| p["key"].as_str().unwrap_or("").to_string())
                .collect();
    ...
    pub async fn get_mcp_server(&self, name: &str) -> Result<Option<Value>, sqlx::Error> {
        let r = sqlx::query(
            "SELECT name,endpoint,auth_json FROM connectors WHERE name=? AND kind='mcp'",
        )
        .bind(name)
        .fetch_optional(&self.pool)
        .await?;
    ...
    pub async fn refresh_mcp_tool(
        ...
        let exists: Option<i32> =
            sqlx::query_scalar("SELECT 1 FROM capabilities WHERE name=? AND kind='mcp'")
                .bind(name)
                .fetch_optional(&self.pool)
                .await?;
    ...
    pub async fn uninstall_mcp(&self, name: &str) -> Result<(), sqlx::Error> {
        sqlx::query("DELETE FROM capabilities WHERE connector_name=? AND kind='mcp'")
            .bind(name)
            .execute(&self.pool)
            .await?;
    ...
    pub async fn uninstall_skill(&self, name: &str) -> Result<(), sqlx::Error> {
        sqlx::query("DELETE FROM capabilities WHERE name=? AND kind='skill'")
            .bind(name)
            .execute(&self.pool)
            .await?;
        Ok(())
    ...
    pub async fn uninstall_subagent(&self, name: &str) -> Result<(), sqlx::Error> {
        sqlx::query("DELETE FROM capabilities WHERE name=? AND kind='subagent'")
            .bind(name)
            .execute(&self.pool)
            .await?;
        Ok(())
    ...
    pub async fn add_capability_request(
        &self,
        capability: &str,
        detail: &str,
        user: &str,
        ...
        let now = now_secs();
        let row = sqlx::query(
            "SELECT id,count,requesters FROM capability_requests WHERE lower(capability)=lower(?)",
        )
        .bind(cap)
        .fetch_optional(&self.pool)
        .await?;
        ...
        if let Some(r) = row {
            ...
            let mut requesters: Vec<String> = serde_json::from_str::<Vec<String>>(
                &r.get::<Option<String>, _>("requesters").unwrap_or_default(),
            )
            .unwrap_or_default();
    ...
    pub async fn list_capability_requests(&self) -> Result<Vec<Value>, sqlx::Error> {
        let rows =
            sqlx::query("SELECT * FROM capability_requests ORDER BY count DESC, updated_at DESC")
                .fetch_all(&self.pool)
                .await?;
        let mut out = Vec::new();
        for r in rows {
    ...
    pub async fn mail_thread(&self, email: &str) -> Result<Vec<Value>, sqlx::Error> {
        let rows =
            sqlx::query("SELECT * FROM mail_messages WHERE waitlist_email=? ORDER BY created_at")
                .bind(email)
                .fetch_all(&self.pool)
                .await?;
        let mut out = Vec::new();
    ...
    pub async fn mail_msgid_seen(&self, msg_id: &str) -> Result<bool, sqlx::Error> {
        ...
        let exists: Option<i32> =
            sqlx::query_scalar("SELECT 1 FROM mail_messages WHERE msg_id=? LIMIT 1")
                .bind(msg_id)
                .fetch_optional(&self.pool)
                .await?;
        Ok(exists.is_some())
    ...
    pub async fn waitlist_email_set(
        &self,
    ) -> Result<std::collections::HashSet<String>, sqlx::Error> {
    ...
    pub async fn record_push_device(
        &self,
        user_id: &str,
        device: &str,
        platform: &str,
